...
def check_eigen(conf, *k, **kw):
    ...
    try:
        ...
        # Testing memory misalignment
        conf.env.DEFINES_EIGEN = []
        # conf.env.DEFINES_EIGEN.append('EIGEN_DONT_VECTORIZE')
        # conf.env.DEFINES_EIGEN.append('EIGEN_DISABLE_UNALIGNED_ARRAY_ASSERT')
        Logs.info(conf.env.DEFINES_EIGEN)
    except:
        if required:
